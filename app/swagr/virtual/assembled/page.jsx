...
export default function SwagrControlledInstantVirtual() {
  ...
  useEffect(() => {
    ...
    if (params.get('source') === 'pinned-compare' && requestedConcept) {
      ...
      if (requestedIndex >= 0) { setCompareReturnContext({ source: 'PINNED_COMPARE', conceptId: requestedConcept.id, conceptName: requestedConcept.name, compareSlot: slotMatches ? rawSlot : requestedIndex + 1, compareSet }); setGalleryFocusId(requestedConcept.id); if (compareTargetValid) setGalleryCompareTargetId(requestedCompareTargetId); }
    }
