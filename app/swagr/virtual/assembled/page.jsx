  ...
  return <main className="min-h-screen" style={{ background: C.bg, color: '#fff' }}>
    ...
    <div className="relative mx-auto max-w-7xl px-5 py-8">
      ...
      {compareReturnContext?.entry === 'STOREFRONT_STAGED_PAIR' && focusedPinnedVirtual && pinnedVirtualCompareTarget && <section data-testid="swagr-storefront-staged-pair-virtual-context" className="mb-6 rounded-3xl border p-5 sm:p-6" style={{ borderColor: `${C.green}66`, background: 'linear-gradient(145deg, rgba(52,211,153,.10), rgba(27,21,48,.98))' }} aria-label="Staged storefront pair controlled virtual context">
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2">{[focusedPinnedVirtual, pinnedVirtualCompareTarget].map((item, index) => <article key={`staged-storefront-pair-${item.concept.id}`} className="overflow-hidden rounded-2xl border" style={{ borderColor: index === 0 ? `${C.gold}55` : `${C.purple}55`, background: C.panel2 }}><div className="p-3 pb-0"><ConceptVisual concept={item.concept} brandName={item.ready ? markText || 'SWAGR' : 'PREVIEW HELD'} placement={placement} markScale={markScale} compact conceptLabel={index === 0 ? 'Staged storefront direction' : 'Staged governed alternative'} /></div><div className="p-4"><div className="flex flex-wrap items-center gap-2"><Pill tone={index === 0 ? 'warn' : 'purple'}>{index === 0 ? 'Storefront direction' : 'Governed alternative'}</Pill><Pill tone={item.ready ? 'good' : 'warn'}>{item.ready ? 'Assembly ready' : 'Preview withheld'}</Pill></div><h3 className="mt-2 text-sm font-black">{item.concept.name}</h3><p className="mt-1 text-[10px]" style={{ color: C.muted }}>{item.concept.id} · {item.concept.category} · {item.assemblyScore}% controlled assembly gate</p></div></article>)}</div>
