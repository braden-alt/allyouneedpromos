...
const TOOLS = [
  {
    ...
    desc: 'Apollo CSV → 4-email sequence per lead → CSV for Instantly',
    color: '#6C47FF',
  },
  {
    href: '/reply',
    icon: Inbox,
    name: 'Reply Handler',
    desc: 'Inbound reply → classify + draft response in brand voice',
    color: '#FF7A1A',
  },
  {
    href: '/quote',
    icon: Calculator,
    name: 'Quote Engine',
    desc: 'Line items + ZIP → all-inclusive PDF quote or email',
    color: '#F5C842',
  },
  {
    href: '/mockup',
    icon: Layers,
    name: 'Mockup Studio',
    desc: 'Product photo + logo → branded PDF proof',
    color: '#00D9B2',
  },
  {
    href: '/ideas',
    icon: Lightbulb,
    name: 'Idea Generator',
    desc: 'Customer brief → 4-12 product ideas → mood-board PDF',
    color: '#E6E1D4',
  },
];
    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="terminal-grid min-h-screen">
        <div className="max-w-5xl mx-auto px-6 py-16">
          ...
          <div className="mb-16">
            <div className="flex items-center gap-3 mb-6">
              <div className="w-12 h-12 rounded-lg flex items-center justify-center"
                   style={{ background: 'linear-gradient(135deg, #6C47FF 0%, #F5C842 100%)', boxShadow: '0 0 24px rgba(108, 71, 255, 0.4)' }}>
              ...
              <div>
                ...
                <div className="font-mono text-[10px] uppercase tracking-[0.2em] text-purple-400/70 mt-1.5">
                  Internal · Braden Forge · HMH Holdings
                </div>
              </div>
            </div>
            <p className="text-zinc-400 max-w-2xl leading-relaxed">
              Five tools. One brain (Claude + 21 skills). Built for the way the business actually runs — not the way software companies think it should.
            </p>
          </div>
              ...
              {TOOLS.map(tool => {
                ...
                return (
                  <Link
                    key={tool.href}
                    href={tool.href}
                    className="group bg-zinc-950/80 border border-purple-900/40 rounded-lg p-5 hover:border-purple-500/60 transition-all hover:-translate-y-0.5"
                  ...
                  >
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="font-display text-lg font-bold text-white leading-tight mb-1 group-hover:text-purple-200 transition-colors">
                          {tool.name}
                        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {EXTERNAL.map(item => (
                <a
                  ...
                  className="bg-zinc-950/60 border border-zinc-800/60 rounded-lg p-4 hover:border-zinc-700 transition-all flex items-center justify-between group"
                >
                  <div>
          ...
          <div className="border-t border-zinc-900 pt-6 font-mono text-[10px] uppercase tracking-[0.15em] text-zinc-600">
            v1.0 · hmh holdings llc · do not share
          </div>
        </div>
      </div>
    </div>
